...
def draw_map(win,player):
    for row in range(8):
        for col in range(8):
            ...
            pg.draw.rect(
                win,
                CONST.FLOOR if CONST.MAP[square] == '#' else CONST.WALL_UNLIT,
...
def draw(win,player):
    ...
    pg.draw.rect(win, CONST.FLOOR_3D, (480, CONST.SCREEN_HEIGHT / 2, CONST.SCREEN_HEIGHT, CONST.SCREEN_HEIGHT))
